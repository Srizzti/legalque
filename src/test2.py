...
def reciprocal_rank_fusion(search_results_dict, k=60):
    try:
        ...
        # Print the reranked results for debugging or verification
...
def generate_content_google(reranked_results, queries):
    content_prompts = [f"Summarize the key information from the following queries and documents in exactly 5 to 6 concise lines and give reply to the user: {queries} and {list(reranked_results.keys())}"]
    response = model.generate_content(content_prompts)
